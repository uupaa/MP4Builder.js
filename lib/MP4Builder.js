...
function _overwrite_stco_chunk_offset(streamObject) { // @arg MP4StreamObject
    ...
    var currentValue = stream[cursor + 0] << 24 |
                       stream[cursor + 1] << 16 |
                       stream[cursor + 2] <<  8 |
                       stream[cursor + 3];
                                                               // | BoxSize   | BoxType   | BoxData |
                                                               // |-----------|-----------|---------|
    var newValue     = mp4Tree["root"]["mdat"]["BoxHead"] + 8; // |00 00 00 00|00 00 00 00|...      |
                                                               // |           |           | ^       |
                                                               // |           |           | this    |
    if (currentValue !== newValue) {
        if (MP4Builder["VERBOSE"]) {
            console.log("overwrite stco.chunk_offset value: 0x" + currentValue.toString(16) + " -> 0x" + newValue.toString(16) + ", position: 0x" + cursor.toString(16));
        }
        stream[cursor + 0] = (newValue >> 24) & 0xff;
        stream[cursor + 1] = (newValue >> 16) & 0xff;
        stream[cursor + 2] = (newValue >>  8) & 0xff;
        stream[cursor + 3] = (newValue >>  0) & 0xff;
    }
    return streamObject;
