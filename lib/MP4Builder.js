...
var MP4Builder = {
    "VERBOSE":  VERBOSE,
    "build":    MP4Builder_build, // MP4Builder.build(tree:MP4BoxTreeObject, options:Object = {}):MP4StreamObject
};
...
function MP4Builder_build(tree,      // @arg MP4BoxTreeObject - { root: { ftyp, moov: {...}, free, mdat } }
                          options) { // @arg Object = {} - { fastStart:Boolean = false, diagnostic:Boolean = false }
                                     ...
                                     // @ret MP4StreamObject - MP4 file stream(ISO Base Media Format) and diagnostic info.
    options = options || {};
    ...
    if (fastStart) {
        return _adjustFastStart(streamObject);
    }
    return streamObject; // { stream, diagnostic }
}

function _adjustFastStart(streamObject) { // @arg MP4StreamObject
                                          // @ret MP4StreamObject
    var stream  = streamObject["stream"];
    var mp4Tree = MP4Parser["parse"](stream);
    var offset  = mp4Tree["root"]["mdat"]["BoxHead"] + 8;
    var cursor  = mp4Tree["root"]["moov"]["trak"][0]["mdia"]["minf"]["stbl"]["stco"]["BoxHead"] + 16;

    if (MP4Builder["VERBOSE"]) {
        console.log("adjustFastStart before", stream[cursor + 0], stream[cursor + 1], stream[cursor + 2], stream[cursor + 3]);
    }
    ...
    if (MP4Builder["VERBOSE"]) {
        console.log("adjustFastStart after", stream[cursor + 0], stream[cursor + 1], stream[cursor + 2], stream[cursor + 3]);
    }
    return streamObject;
}
...
function _hdlr(tree,       // @arg MP4BoxTreeObject
    ...
    var view = _createFullBoxClassView( _getLastBoxType(boxPath), diagnostic, boxPath,
                                        box["version"], box["flags"] );
    _write4(view, 0);                               // pre_defined = 0
    _writeT(view, box["handler_type"],  4);         // "vide"

    switch (typeof box["handler_type2"]) {
    case "number": _write4(view, box["handler_type2"]);    break; // "appl"
    case "string": _writeT(view, box["handler_type2"], 4); break; // "appl"
    }
